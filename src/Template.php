...
class Template implements ITemplate {
	...
	public function setBlock($block, $vars = array()) {
		...
		$varvals =  array_map(function($v) {return str_replace(array('\\', '$'), array('\\\\', '\$'), $v);}, array_values($vars));
	...
	public function getRender() {
		...
		$varvals = array_map(function($v) {return str_replace(array('\\', '$'), array('\\\\', '\$'), $v);}, array_values($this->vars));
		$search = array_map(function($v) {return '/(?:{' . $v . '(?:\/[^{}\r\n]*)*})|(?:{"' . $v . '"})/';}, $varkeys);
